<template>
  <div>
    ...
    <ul class="dishes-list">
      ...
      <li :class="returnClass(dish)" class="dish-item" v-for="dish in dishes" :key="dish.key">
        <p class="dish-name">{{dish.name}}</p>
        <p class="dish-counter" v-if="dish.costSmall===0"></p>
        <p
          class="dish-counter"
          v-if="dish.costSmall!==0"
        >{{dish.counterSmall}} ({{dish.costSmall}}тг)</p>
        <p class="dish-counter">{{dish.counterStand}} ({{dish.costStand}}тг)</p>
